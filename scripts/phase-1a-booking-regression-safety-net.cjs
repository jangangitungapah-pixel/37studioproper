...
const TEST_PATH = path.join(ROOT, 'scripts', 'booking-lifecycle-regression-test.mjs');
...
function writeIfChanged(filePath, content) {
  ...
  if (fs.existsSync(filePath)) {
    ...
    if (current === normalizedContent) {
      console.log(`[phase-1a] Unchanged: ${path.relative(ROOT, filePath)}`);
      return false;
    }
  ...
  console.log(`[phase-1a] Updated: ${path.relative(ROOT, filePath)}`);
  return true;
}

if (!fs.existsSync(PACKAGE_PATH)) {
  fail('package.json tidak ditemukan. Jalankan script dari root repository.');
}

const packageJson = JSON.parse(fs.readFileSync(PACKAGE_PATH, 'utf8'));

if (!packageJson.scripts || typeof packageJson.scripts.test !== 'string') {
  fail('scripts.test tidak ditemukan di package.json.');
}
...
if (fs.existsSync(TEST_PATH)) {
  const existingTest = fs.readFileSync(TEST_PATH, 'utf8').replace(/\r\n/g, '\n');

  if (existingTest !== testSource.replace(/\r\n/g, '\n')) {
    fail(
      'scripts/booking-lifecycle-regression-test.mjs sudah ada dengan isi berbeda. ' +
      'Tidak dioverwrite otomatis demi keamanan.',
    );
  }
} else {
  writeIfChanged(TEST_PATH, testSource);
}

const regressionCommand = 'node scripts/booking-lifecycle-regression-test.mjs';

const currentTestCommands = packageJson.scripts.test
  .split('&&')
  .map((command) => command.trim())
  .filter(Boolean);

if (!currentTestCommands.includes(regressionCommand)) {
  backupFile(PACKAGE_PATH);
  ...
  console.log('[phase-1a] Updated: package.json');
} else {
  console.log('[phase-1a] package.json already contains booking regression test.');
}

console.log('');
console.log('✅ Phase 1A patch prepared.');
console.log('   - Added booking lifecycle regression test');
console.log('   - Added regression test to npm test');
console.log('   - No application UI changed');
console.log('   - No Firestore schema/rules changed');
console.log('   - No production business data migrated');