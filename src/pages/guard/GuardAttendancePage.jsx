  ...
  return (
    <main className="guard-shift-page">
      <section className="guard-shift-shell">
        ...
        {isReady && !authUser ? (
          <section
            aria-label="Guard Login Entry"
            className="guard-shift-card"
          >
            <div className="guard-shift-title">
              <strong>Masuk ke Guard Portal</strong>
              <small>
                Gunakan login akun bersama agar metode autentikasi tetap
            ...
            <small>
              Email/password, Google, dan OTP nomor HP tersedia dari satu
              session Firebase yang sama. Login ini tidak membuat role Guard
        ...
        {isReady && authUser && isWrongPortalClient ? (
          ...
          >
            <ShieldCheck size={24} />
            <strong>Akun Client tidak menggunakan Guard Portal</strong>
            <p>
              Anda login sebagai Client. Guard Portal hanya untuk penjaga
              operasional, Owner oversight, dan konteks Admin yang sesuai.
            </p>
            <Link
              className="guard-shift-main-button"
              to="/client/portal"
        ...
        {isReady && authUser && isBlockedGuardAccess ? (
          ...
          >
            <AlertCircle size={24} />
            <strong>
              {guardAccount?.role === 'studio_guard'
            ...
            <p>
              {guardAccount?.role === 'studio_guard'
                ? guardAccount?.status === 'rejected'
                  ? 'Akun Guard ini telah ditolak atau dinonaktifkan. Hubungi Owner sebelum mencoba Clock In/Out.'
                  : 'Akun Guard ini masih menunggu status approved dari Owner.'
                : guardAccount?.role === 'admin'
                  ? guardAccount?.status === 'rejected'
                    ? 'Akun Admin ini telah ditolak atau dinonaktifkan dan tidak mendapat akses operasional Guard.'
                    : 'Akun Admin ini belum approved. Guard Portal tidak mengubah status atau role akun.'
                  : 'Status akun ini belum memenuhi syarat untuk menggunakan Guard Portal.'}
            </p>
            ...
            {guardAccount?.role === 'admin' ? (
              ...
              <small>
                Hubungi Owner untuk memeriksa status akun. Tidak ada role yang
                diubah otomatis dari halaman ini.
              </small>
            )}
          </section>
        ) : null}
        ...
        {isReady && authUser && isInvalidGuardAccess ? (
          ...
          >
            <AlertCircle size={24} />
            <strong>Data akun belum dapat digunakan</strong>
            <p>
              Identitas akun belum lengkap atau role akun tidak dikenali oleh
              Guard Portal. Tidak ada kontrol attendance yang dibuka.
            </p>
            <small>
              Hubungi Owner untuk memeriksa User &amp; Access Settings, lalu
